...
private:
  ...
  Eigen::Vector3f compute_plane_normal(const pcl::ModelCoefficients::Ptr& coeff,
                                       const Eigen::Vector3f& ray_direction) const;
  Eigen::Vector3f find_ray_plane_intersection(const pcl::ModelCoefficients::Ptr& coeff,
                                              const Eigen::Vector3f& ray_direction) const;
  Eigen::Vector3f shift_point_along_normal(const Eigen::Vector3f& intersection_point,
                                           const Eigen::Vector3f& plane_normal) const;
  Eigen::Matrix3f create_rotation_matrix(const pcl::ModelCoefficients::Ptr& coefficients,
