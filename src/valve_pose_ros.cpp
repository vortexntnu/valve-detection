// valve_pose_ros.cpp
// ROS node: subscribes to depth, color, and detections; publishes valve poses
// and visualizations.
...
// Declares all ROS parameters, initializes the pose estimator, and sets up
...
ValvePoseNode::ValvePoseNode(const rclcpp::NodeOptions& options)
    ...
    const auto pose_stamped_topic =
        declare_parameter<std::string>("valve_pose_stamped_pub_topic");
    const auto pose_topic =
        declare_parameter<std::string>("valve_poses_pub_topic");
    const auto lm_topic = declare_parameter<std::string>("landmarks_pub_topic");
    const auto img_topic =
        declare_parameter<std::string>("processed_image_pub_topic");
    const auto depth_color_topic =
        declare_parameter<std::string>("depth_colormap_pub_topic");
    const auto valve_points_topic =
        declare_parameter<std::string>("valve_points_pub_topic");
    const auto depth_cloud_topic =
        declare_parameter<std::string>("depth_cloud_pub_topic");

    depth_colormap_vmin_ = static_cast<float>(
        declare_parameter<double>("depth_colormap_value_min"));
    depth_colormap_vmax_ = static_cast<float>(
        declare_parameter<double>("depth_colormap_value_max"));
    ...
    cv_camera_matrix_ = (cv::Mat_<double>(3, 3) << color_props_.intr.fx, 0,
                         color_props_.intr.cx, 0, color_props_.intr.fy,
                         color_props_.intr.cy, 0, 0, 1);

    cv_dist_coeffs_ =
        (cv::Mat_<double>(5, 1) << declare_parameter<double>("color_d1"),
    ...
    RCLCPP_INFO(
        ...
        color_props_.intr.fx, color_props_.intr.fy, color_props_.intr.cx,
        color_props_.intr.cy);
    ...
    RCLCPP_INFO(
        ...
        depth_props_.intr.fx, depth_props_.intr.fy, depth_props_.intr.cx,
        depth_props_.intr.cy);
    ...
    pose_stamped_pub_ = create_publisher<geometry_msgs::msg::PoseStamped>(
        pose_stamped_topic, qos);
    pose_pub_ =
    ...
    valve_points_pub_ = create_publisher<sensor_msgs::msg::PointCloud2>(
        valve_points_topic, qos);
    depth_cloud_pub_ =
        create_publisher<sensor_msgs::msg::PointCloud2>(depth_cloud_topic, qos);

    if (debug_visualize_) {
        const auto ann_topic =
            declare_parameter<std::string>("debug_annulus_pub_topic");
        const auto pln_topic =
            declare_parameter<std::string>("debug_plane_pub_topic");
        annulus_pub_ =
...
BoundingBox ValvePoseNode::to_bbox(
    const vision_msgs::msg::BoundingBox2D& b) const {
    BoundingBox o;
...
BoundingBox ValvePoseNode::undistort_bbox(const BoundingBox& bbox) const {
    if (cv_camera_matrix_.empty() || cv_dist_coeffs_.empty())
        return bbox;

    std::vector<cv::Point2f> pts{{bbox.center_x, bbox.center_y}};
    std::vector<cv::Point2f> undistorted;
    cv::undistortPoints(pts, undistorted, cv_camera_matrix_, cv_dist_coeffs_,
...
std::vector<size_t> ValvePoseNode::filter_duplicate_detections(
    ...
    for (size_t si = 0; si < scored.size() && kept.size() < 2; ++si) {
        ...
        for (size_t sj = si + 1; sj < scored.size(); ++sj) {
            ...
            const float iom =
                inter / std::min(ai, aj);  // intersection over minimum
...
// Packs poses into a LandmarkArray message with type/subtype fields and
// publishes it.
...
void ValvePoseNode::sync_cb(
    ...
    cv::Mat color_vis;
    const bool publish_image =
        color && image_pub_->get_subscription_count() > 0;
    if (publish_image) {
        cv_bridge::CvImageConstPtr cv_color =
            cv_bridge::toCvShare(color, "bgr8");
    ...
    if (det->detections.empty()) {
        ...
        if (publish_image) {
            image_pub_->publish(
                *cv_bridge::CvImage(color->header, "bgr8", color_vis)
                     .toImageMsg());
        }
        return;
    }
    ...
    std::vector<BoundingBox>
        raw_boxes;  // raw detection coords for depth colormap
    std::vector<Pose> poses;

    for (size_t idx : kept) {
        // Parse bounding box (YOLO-space), map to original image, then
        // undistort.
        ...
        Pose pose;
        if (detector_->compute_pose_from_depth(depth_img, yolo_box, pose,
                                               ann_dbg, pln_dbg, true)) {
    ...
    if (publish_colormap) {
        for (size_t i = 0; i < raw_boxes.size(); ++i) {
            ...
            // Project each OBB corner from color image space to depth image
            // space using the full intrinsic + extrinsic pipeline. Fall back to
            // the raw color pixel when no valid pose depth is available.
