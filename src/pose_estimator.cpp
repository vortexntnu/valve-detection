...
// Initializes YOLO dimensions, annulus ratio, RANSAC parameters, and handle offset.
...
// Computes letterbox scale factor and padding from the color image and YOLO dimensions.
...
// Remaps a bounding box from YOLO letterbox space back to original image coordinates.
BoundingBox PoseEstimator::transform_bounding_box(const BoundingBox& bbox) const {
  BoundingBox transformed = bbox;
  transformed.center_x = (bbox.center_x - letterbox_pad_x_) / letterbox_scale_factor_;
  transformed.center_y = (bbox.center_y - letterbox_pad_y_) / letterbox_scale_factor_;
  transformed.size_x  /= letterbox_scale_factor_;
  transformed.size_y  /= letterbox_scale_factor_;
  return transformed;
}

// Fits a plane to the point cloud using RANSAC; returns false if no inliers are found.
...
// Returns the normalized 3D ray direction from the camera origin through the bbox center.
Eigen::Vector3f PoseEstimator::get_ray_direction(const BoundingBox& bbox) const {
  const float xc = (bbox.center_x - static_cast<float>(color_image_properties_.intr.cx))
                   / static_cast<float>(color_image_properties_.intr.fx);
  const float yc = (bbox.center_y - static_cast<float>(color_image_properties_.intr.cy))
    ...
    const Eigen::Vector3f& ray_direction) const
{
  ...
  if (normal.dot(ray_direction) > 0.0f) normal = -normal;
  return normal;
}
...
// Builds a 3×3 rotation matrix from the plane normal (Z) and the projected bbox angle (X).
    ...
    float angle) const
{
  ...
  if (filter_direction_.dot(x_axis) < 0) x_axis = -x_axis;
...
// Extracts a point cloud from the depth image, fits a plane, and returns the valve pose.
    ...
    bool debug_visualize) const
{
  ...
  if (has_depth_props_) {
    extract_bbox_pcl_aligned(
        depth_image, bbox_org,
        color_image_properties_, depth_image_properties_,
  ...
  if (cloud->points.size() < 4) return false;

  if (debug_visualize && annulus_dbg) *annulus_dbg += *cloud;

  pcl::ModelCoefficients::Ptr coeff(new pcl::ModelCoefficients);
  pcl::PointIndices::Ptr inliers(new pcl::PointIndices);
  if (!segment_plane(cloud, coeff, inliers)) return false;
  ...
  const Eigen::Matrix3f rot = create_rotation_matrix(coeff, normal, bbox_org.theta);
