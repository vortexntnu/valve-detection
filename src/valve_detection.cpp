...
ValveDetectionNode::ValveDetectionNode(const rclcpp::NodeOptions& options)
    ...
    sync_->registerCallback(std::bind(
        &ValveDetectionNode::synchronized_callback, this, _1, _2, _3));
    ...
    camera_info_subscription_ =
        this->create_subscription<sensor_msgs::msg::CameraInfo>(
            ...
            std::bind(&ValveDetectionNode::camera_info_callback, this,
                      std::placeholders::_1));

    processed_image_pub_ = this->create_publisher<sensor_msgs::msg::Image>(
        "yolov8_valve_detection_image", 10);
    plane_normal_pub_ =
        this->create_publisher<geometry_msgs::msg::Vector3>("plane_normal", 10);
...
//         cv::Mat cv_depth_image = cv_bridge::toCvCopy(depth_image,
//         "32FC1")->image; cv::Mat cv_color_image =
//         cv_bridge::toCvCopy(color_image, "bgr8")->image;

//         if (!detections->detections.empty())
//         {
//             const auto *first_detection = &detections->detections[0];

//             // The most centred of the two most likely variables are chosen
//             as a boundingbox.
...
//             {
//                 float dist_first =
//                 std::sqrt(std::pow(detections->detections[0].bbox.center.position.x
//                 - cx, 2) +
//                                             std::pow(detections->detections[0].bbox.center.position.y
//                                             - cy, 2));

//                 float dist_second =
//                 std::sqrt(std::pow(detections->detections[1].bbox.center.position.x
//                 - cx, 2) +
//                                             std::pow(detections->detections[1].bbox.center.position.y
//                                             - cy, 2));
...
//             int center_x =
//             static_cast<int>(first_detection->bbox.center.position.x /
//             this->width_scalar_); int center_y =
//             static_cast<int>((first_detection->bbox.center.position.y - 140)
//             / this->height_scalar_); int width =
//             static_cast<int>(first_detection->bbox.size_x /
//             this->width_scalar_); int height =
//             static_cast<int>(first_detection->bbox.size_y /
//             this->height_scalar_);

//             pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new
//             pcl::PointCloud<pcl::PointXYZ>); bool points_added = false;

//             for (int y = center_y - (height / 2); y <= center_y + (height /
//             2); ++y)
//             {
//                 for (int x = center_x - (width / 2); x <= center_x + (width /
//                 2); ++x)
//                 {
//                     int dx = x - center_x;
//                     int dy = y - center_y;
//                     if (dx * dx / (float)((width / 2) * (width / 2)) + dy *
//                     dy / (float)((height / 2) * (height / 2)) <= 1.0)
//                     {
//                         if (y >= 0 && y < cv_depth_image.rows && x >= 0 && x
//                         < cv_depth_image.cols)
//                         {
//                             float depth_value = cv_depth_image.at<float>(y,
//                             x); pcl::PointXYZ point; project_pixel_to_3d(x,
//                             y, depth_value, point);
//                             // if (y == center_y && x == center_x)
//                             // {
//                             //     RCLCPP_INFO(this->get_logger(), "center 2d
//                             point: (%d, %d, %f)", x, y, depth_value);
//                             //     RCLCPP_INFO(this->get_logger(), "Center 3d
//                             point: (%.3f, %.3f, %.3f)", point.x, point.y,
//                             point.z);
//                             // }
...
//                 pcl::PointIndices); seg.setInputCloud(cloud);
...
//                     RCLCPP_INFO(this->get_logger(), "RANSAC successful: Plane
//                     normal (%.3f, %.3f, %.3f)",
//                                  coefficients->values[0],
//                                  coefficients->values[1],
//                                  coefficients->values[2]);

//                     pcl::PointCloud<pcl::PointXYZ>::Ptr near_plane_cloud(new
//                     pcl::PointCloud<pcl::PointXYZ>);

//                     // Select points within a distance threshold of the plane
//                     model pcl::ExtractIndices<pcl::PointXYZ> extract;
//                     extract.setInputCloud(cloud);
//                     extract.setIndices(inliers);
//                     extract.setNegative(false);  // Select the inliers
//                     extract.filter(*near_plane_cloud);

//                     // Now we can use `selectWithinDistance` to select
//                     inliers that are within the distance threshold
//                     pcl::PointIndices::Ptr selected_inliers(new
//                     pcl::PointIndices()); float threshold_distance = 0.02; //
//                     Example threshold (3 cm)

//                     // Logic for selecting points close to the
//                     threshold_distance of the plane. for (size_t i = 0; i <
//                     cloud->points.size(); ++i) {
//                         const auto& point = cloud->points[i];
//                         float distance = std::abs(coefficients->values[0] *
//                         point.x +
//                                                 coefficients->values[1] *
//                                                 point.y +
//                                                 coefficients->values[2] *
...
//                         }
//                     }
//                     RCLCPP_INFO(this->get_logger(), "Selected inlier size:
//                     %zu", selected_inliers->indices.size());

//                     // Extract the selected inliers
//                     pcl::PointCloud<pcl::PointXYZ>::Ptr
//                     final_inliers_cloud(new pcl::PointCloud<pcl::PointXYZ>);
...
//                             project_3d_to_pixel(point.x, point.y, point.z, u,
//                             v);

//                             // Check if the point (u, v) is inside the
//                             smaller ellipse int dx = u - center_x; int dy = v
//                             - center_y;

//                             // Check if point is inside the smaller ellipse
//                             in image space float a =
//                             static_cast<float>(width) / 6.0f;  // Semi-major
//                             axis float b = static_cast<float>(height) / 6.0f;
//                             // Semi-minor axis

//                             if ((dx * dx) / (a * a) + (dy * dy) / (b * b)
//                             <= 1.0f) {
//                                 // Point is inside the ellipse, add it to the
//                                 near_plane_cloud
...
//                         centroid += Eigen::Vector3f(point.x, point.y,
//                         point.z);
//                     }
//                     centroid /= near_plane_cloud->points.size();
//                 } else {
//                     RCLCPP_WARN(this->get_logger(), "No valid near plane
//                     points found."); return;
//                 }

//                 int x1 = std::max(center_x - width / 2, 0);
//                 int y1 = std::max(center_y - height / 2, 0);
//                 int x2 = std::min(center_x + width / 2, cv_color_image.cols -
//                 1); int y2 = std::min(center_y + height / 2,
//                 cv_color_image.rows - 1);

//                 // Extract ROI (region of interest) from the color image
//                 cv::Mat roi = cv_color_image(cv::Rect(x1, y1, x2 - x1, y2 -
//                 y1));
...
//                     {
//                         double length = std::hypot(line[2] - line[0], line[3]
//                         - line[1]); if (length > max_length)
//                         {
//                             max_length = length;
//                             longest_line = line;
//                         }
...
//                     double angle = std::atan2(longest_line[3] -
//                     longest_line[1], longest_line[2] - longest_line[0]);
...
//                     quaternion.setFromTwoVectors(Eigen::Vector3f(1, 0, 0),
//                     line_direction.normalized());

//                     if (std::isnan(quaternion.x()) ||
//                     std::isnan(quaternion.y()) ||
//                         std::isnan(quaternion.z()) ||
//                         std::isnan(quaternion.w())) {
//                         RCLCPP_WARN(this->get_logger(), "Invalid quaternion
//                         computed, skipping pose publishing."); return;
//                     }
...
//                     RCLCPP_INFO(this->get_logger(), "Detected line
//                     published.");
//                 }
//                 else
//                 {
//                     RCLCPP_WARN(this->get_logger(), "No lines detected in
//                     bounding box.");
//                 }
//                 }
//             else
...
void ValveDetectionNode::process_and_publish_image(
    ...
    try {
        ...
        if (!detections->detections.empty()) {
            const auto* first_detection = &detections->detections[0];
            // The most centred of the two most likely variables are chosen as a
            // boundingbox. Calculate distance to center (cx, cy)
            double cx = camera_info_->k[2];  // k[2] = cx
            double cy = camera_info_->k[5];  // k[5] = cy
            if (detections->detections.size() > 1) {
                float dist_first = std::sqrt(
                    std::pow(
                        detections->detections[0].bbox.center.position.x - cx,
                        2) +
                    std::pow(
                        detections->detections[0].bbox.center.position.y - cy,
                        2));

                float dist_second = std::sqrt(
                    std::pow(
                        detections->detections[1].bbox.center.position.x - cx,
                        2) +
                    std::pow(
                        detections->detections[1].bbox.center.position.y - cy,
                        2));
            ...
            int center_x = static_cast<int>(
                first_detection->bbox.center.position.x / this->width_scalar_);
            int center_y = static_cast<int>(
                (first_detection->bbox.center.position.y - 140) /
                this->height_scalar_);
            ...
            for (int y = center_y - (height / 2); y <= center_y + (height / 2);
                 ++y) {
                for (int x = center_x - (width / 2);
                     x <= center_x + (width / 2); ++x) {
                    int dx = x - center_x;
                    int dy = y - center_y;
                    if (dx * dx / (float)((width / 2) * (width / 2)) +
                            dy * dy / (float)((height / 2) * (height / 2)) <=
                        1.0) {
                        if (y >= 0 && y < cv_image.rows && x >= 0 &&
                            x < cv_image.cols) {
            ...
            if (points_added && cloud->points.size() > 0) {
                ...
                if (inliers->indices.size() > 0) {
                    ...
                    RCLCPP_INFO(
                        this->get_logger(),
                        "RANSAC successful: Plane normal (%.3f, %.3f, %.3f)",
                        coefficients->values[0], coefficients->values[1],
                        coefficients->values[2]);

                    pcl::PointCloud<pcl::PointXYZ>::Ptr near_plane_cloud(
                        new pcl::PointCloud<pcl::PointXYZ>);
                    ...
                    // Now we can use `selectWithinDistance` to select inliers
                    // that are within the distance threshold
                    pcl::PointIndices::Ptr selected_inliers(
                        new pcl::PointIndices());
                    float threshold_distance =
                        0.02;  // Example threshold (3 cm)

                    // Logic for selecting points close to the
                    // threshold_distance of the plane.
                    ...
                    RCLCPP_INFO(this->get_logger(), "Selected inlier size: %zu",
                                selected_inliers->indices.size());

                    // Extract the selected inliers
                    pcl::PointCloud<pcl::PointXYZ>::Ptr final_inliers_cloud(
                        new pcl::PointCloud<pcl::PointXYZ>);
                    pcl::PointCloud<pcl::PointXYZ>::Ptr near_plane_cloud_copy(
                        new pcl::PointCloud<pcl::PointXYZ>);
                    ...
                    for (const auto& idx : selected_inliers->indices) {
                        ...
                        // Check if the point (u, v) is inside the smaller
                        // ellipse
                        int dx = u - center_x;
                        int dy = v - center_y;

                        // Check if point is inside the smaller ellipse in image
                        // space
                        float a = static_cast<float>(width) /
                                  6.0f;  // Semi-major axis
                        float b = static_cast<float>(height) /
                                  6.0f;  // Semi-minor axis

                        if ((dx * dx) / (a * a) + (dy * dy) / (b * b) <= 1.0f) {
                            // Point is inside the ellipse, add it to the
                            // near_plane_cloud
                            final_inliers_cloud->points.push_back(
                                cloud->points[idx]);
                        }
                    }
                    ...
                    RCLCPP_INFO(this->get_logger(),
                                "near_plane_points size: %zu",
                                near_plane_cloud->points.size());
                    ...
                    if (near_plane_cloud->points.size() > 0) {
                        ...
                        near_plane_cloud_pub_->publish(
                            near_plane_cloud_msg);  // Publish near plane cloud
                        ...
                        pcl::computeCovarianceMatrixNormalized(
                            *near_plane_cloud, centroid, covariance);
                        ...
                        if (filter_direction_.dot(line_direction) < 0) {
                            line_direction =
                                -line_direction;  // Flip to maintain
                                                  // consistency
                        }
                        ...
                        line_pose_pub_->publish(line_pose_msg);
                        RCLCPP_INFO(this->get_logger(),
                                    "Line fitted and published.");

                        // Publish line points as a PointCloud2 message
                        pcl::PointCloud<pcl::PointXYZ>::Ptr line_cloud(
                            new pcl::PointCloud<pcl::PointXYZ>);
                        const int num_points =
                            100;  // Number of points along the line

                        for (int i = -num_points / 2; i <= num_points / 2;
                             ++i) {
                            pcl::PointXYZ line_point;
                            float scale =
                                i * 0.01;  // Adjust the scale for length

                            line_point.x =
                                centroid[0] + scale * line_direction[0];
                            line_point.y =
                                centroid[1] + scale * line_direction[1];
                            line_point.z =
                                centroid[2] + scale * line_direction[2];
                        ...
                        // RCLCPP_INFO(this->get_logger(), "Line points
                ...
                RCLCPP_WARN(this->get_logger(),
                            "No valid points found for RANSAC.");
            }
        }

        auto processed_msg =
            cv_bridge::CvImage(image->header, "32FC1", cv_image).toImageMsg();
