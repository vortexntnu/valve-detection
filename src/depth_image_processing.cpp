...
// Depth-to-3D back-projection, point cloud extraction, and color-to-depth pixel projection.
...
// Back-projects a depth pixel (u, v, depth) to a 3D point using camera intrinsics.
...
// Extracts depth points that fall inside an elliptic annulus around the bbox center (no alignment).
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr& out)
{
  ...
  if (outer_rx < 2.0f || outer_ry < 2.0f) return;
  ...
  for (int v = v0; v <= v1; ++v) {
    if (v < 0 || v >= depth_image.rows) continue;
    for (int u = u0; u <= u1; ++u) {
      if (u < 0 || u >= depth_image.cols) continue;
      ...
      if (!inside_outer || !outside_inner) continue;

      const float z = depth_image.at<float>(v, u);
      pcl::PointXYZ p;
      project_pixel_to_point(
          u, v, z,
          img_props.intr.fx, img_props.intr.fy,
          img_props.intr.cx, img_props.intr.cy,
          p);
...
// Like extract_annulus_pcl but transforms depth pixels into the color frame before the annulus test.
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr& out)
{
  ...
  const float scale = (depth_props.intr.fx > 0.0 && color_props.intr.fx > 0.0)
      ? static_cast<float>(depth_props.intr.fx / color_props.intr.fx)
      : 1.0f;
  constexpr int kMargin = 30;

  const int u0_d = std::max(0,
      static_cast<int>((cx_c - outer_rx) * scale) - kMargin);
  const int u1_d = std::min(depth_image.cols - 1,
      static_cast<int>((cx_c + outer_rx) * scale) + kMargin);
  const int v0_d = std::max(0,
      static_cast<int>((cy_c - outer_ry) * scale) - kMargin);
  const int v1_d = std::min(depth_image.rows - 1,
      static_cast<int>((cy_c + outer_ry) * scale) + kMargin);

  for (int v_d = v0_d; v_d <= v1_d; ++v_d) {
    for (int u_d = u0_d; u_d <= u1_d; ++u_d) {
      const float z_d = depth_image.at<float>(v_d, u_d);
      if (z_d <= 0.0f || std::isnan(z_d) || std::isinf(z_d)) continue;

      // Back-project using depth intrinsics.
      Eigen::Vector3f P_d;
      P_d.x() = static_cast<float>((u_d - depth_props.intr.cx) * z_d / depth_props.intr.fx);
      P_d.y() = static_cast<float>((v_d - depth_props.intr.cy) * z_d / depth_props.intr.fy);
      P_d.z() = z_d;

      // Transform into color camera frame.
      const Eigen::Vector3f P_c = extr.R * P_d + extr.t;
      if (P_c.z() <= 0.0f) continue;
      ...
      if (dxo*dxo + dyo*dyo > 1.0f) continue;  // outside outer ellipse

      const float dxi = (u_c - cx_c) / inner_rx;
      const float dyi = (v_c - cy_c) / inner_ry;
      if (dxi*dxi + dyi*dyi <= 1.0f) continue;  // inside inner ellipse
...
// Extracts depth points whose color-frame projection falls inside the oriented bounding box.
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr& out)
{
  ...
  // Pre-compute the OBB axes in color-image space for a fast rotated-rect test.
  ...
  for (int v_d = v0_d; v_d <= v1_d; ++v_d) {
    for (int u_d = u0_d; u_d <= u1_d; ++u_d) {
      ...
      // Rotate into the OBB local frame and test against the half-extents.
      ...
      if (std::abs(local_x) > half_w || std::abs(local_y) > half_h) continue;
...
// Projects a color image pixel to depth image coordinates using the full intrinsic + extrinsic pipeline.
    ...
    const DepthColorExtrinsic& extr)
{
  ...
  if (Pd.z() <= 0.0f) return {u_c, v_c};  // degenerate – return original

  // Project into depth image.
  const float u_d = static_cast<float>(depth_props.intr.fx) * Pd.x() / Pd.z()
                    + static_cast<float>(depth_props.intr.cx);
  const float v_d = static_cast<float>(depth_props.intr.fy) * Pd.y() / Pd.z()
